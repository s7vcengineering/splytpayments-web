...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "chat_messages", filter: `thread_id=eq.${selectedThread.id}` },
        (payload) => {
    ...
    <div className="flex h-full">
      ...
      <div className={`w-full sm:w-80 lg:w-96 border-r border-gray-200 bg-white flex flex-col ${selectedThread ? "hidden sm:flex" : "flex"}`}>
        <div className="p-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-0.5">{threads.length} conversation{threads.length !== 1 ? "s" : ""}</p>
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">Join a split to start chatting</p>
            </div>
          ) : (
            threads.map((thread) => (
              <button
                ...
                className={`w-full flex items-center gap-3 p-4 text-left hover:bg-gray-50 transition-colors border-b border-gray-50 ${
                  selectedThread?.id === thread.id ? "bg-ocean-50" : ""
                }`}
              >
                <div className="w-11 h-11 rounded-full bg-ocean-100 flex items-center justify-center text-ocean-600 font-bold text-sm shrink-0">
                  {thread.is_direct_message
                    ? thread.dm_partner_avatar_url
                      ? <img src={thread.dm_partner_avatar_url} alt="" className="w-full h-full rounded-full object-cover" />
                      : "DM"
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-medium text-gray-900 truncate">
                      {thread.experience_title || "Direct Message"}
                    </p>
                    <span className="text-xs text-gray-400 shrink-0">{timeAgo(thread.updated_at)}</span>
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {thread.member_ids?.length || 0} member{(thread.member_ids?.length || 0) !== 1 ? "s" : ""}
                    {thread.amount_pledged ? ` · ${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(thread.amount_pledged)} pledged` : ""}
                  </p>
                </div>
              </button>
            ))
          ...
          <>
            ...
            <div className="flex items-center gap-3 p-4 bg-white border-b border-gray-200">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-900 truncate">{selectedThread.experience_title || "Direct Message"}</p>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                      <div className={`flex items-end gap-2 max-w-[75%] ${isMine ? "flex-row-reverse" : ""}`}>
                        ...
                        <div>
                          {!isMine && (
                            <p className="text-xs text-gray-400 mb-0.5 ml-1">{msg.sender_name || "User"}</p>
                          )}
            ...
            <form onSubmit={handleSend} className="p-4 bg-white border-t border-gray-200">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-ocean-500/20 focus:border-ocean-500 transition-all"
                />
                <button
                  type="submit"
                  disabled={sending || !newMessage.trim()}
                  className="p-2.5 bg-ocean-500 text-white rounded-xl hover:bg-ocean-600 transition-colors disabled:opacity-50"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
