...
function normalizeMvpYacht(row: any): Listing {
  ...
  return {
    ...
    photoUrls: (row.photo_urls || []).slice(0, 5),
    features: [
      row.length_feet ? `${row.length_feet}ft` : null,
      row.builder,
      row.cruising_capacity ? `Up to ${row.cruising_capacity} guests` : null,
    ].filter(Boolean),
    source: 'mvpmiami',
  };
}
...
function normalizeCar(row: any): Listing {
  ...
  return {
    ...
    photoUrls: (row.photo_urls || []).slice(0, 5),
    features: [
      row.horsepower ? `${row.horsepower} HP` : null,
      row.zero_to_sixty ? `0-60: ${row.zero_to_sixty}s` : null,
      row.top_speed ? `${row.top_speed} mph` : null,
      row.transmission,
...
function normalizeMansion(row: any): Listing {
  ...
  return {
    ...
    photoUrls: (row.photo_urls || []).slice(0, 5),
    features: [
      row.bedrooms ? `${row.bedrooms} bed` : null,
      row.bathrooms ? `${row.bathrooms} bath` : null,
      ...(row.amenities || []),
    ].filter(Boolean).slice(0, 4),
    source: 'mvpmiami',
  };
...
function normalizeStay(row: any): Listing {
  ...
  return {
    ...
    features: [
      row.property_type,
      row.bedrooms ? `${row.bedrooms} bed` : null,
      row.is_superhost ? 'Superhost' : null,
      ...(row.amenities || []),
    ].filter(Boolean).slice(0, 4),
    source: 'airbnb',
  };
...
function normalizeExperience(row: any): Listing {
  ...
  return {
    ...
    features: [
      row.category,
      row.duration_minutes ? `${Math.round(row.duration_minutes / 60)}h` : null,
      row.host_name ? `Hosted by ${row.host_name}` : null,
    ].filter(Boolean).slice(0, 4),
    source: 'airbnb',
  };
}
...
export default async function handler(req: Request) {
  ...
  if (category === 'all' || category === 'yacht') {
    queries.push(
      queryTable('boats', 'id,name,city,region,hourly_rate,capacity,rating,review_count,photo_urls,amenities,features', cityFilter)
        .then(rows => rows.map(normalizeBoat)),
      queryTable('mvp_yachts', 'id,name,city,region,cruising_capacity,photo_urls,length_feet,builder', cityFilter)
        .then(rows => rows.map(normalizeMvpYacht)),
    );
  }
  if (category === 'all' || category === 'car') {
    queries.push(
      queryTable('exotic_cars', 'id,title,year,make,model,city,region,daily_rate,photo_urls,horsepower,zero_to_sixty,top_speed,transmission,source_provider', cityFilter)
        .then(rows => rows.map(normalizeCar)),
    );
  }
  if (category === 'all' || category === 'stay') {
    queries.push(
      queryTable('mansions', 'id,name,city,region,nightly_rate,capacity,photo_urls,amenities,bedrooms,bathrooms', cityFilter)
        .then(rows => rows.map(normalizeMansion)),
      queryTable('airbnb_stays', 'id,title,city,region,nightly_rate,max_guests,rating,review_count,photo_urls,bedrooms,property_type,is_superhost,amenities', cityFilter)
        .then(rows => rows.map(normalizeStay)),
    );
  }
  if (category === 'all' || category === 'experience') {
    queries.push(
      queryTable('airbnb_experiences', 'id,title,city,region,price_amount,max_guests,rating,review_count,photo_urls,category,duration_minutes,host_name', cityFilter)
        .then(rows => rows.map(normalizeExperience)),
    );
  }
  ...
  // Filter out listings with no photos
  ...
  // Extract available cities for filter UI
